...
class JsonLogFormatter(JsonFormatter):  # pragma: no cover
    def add_fields(self, log_record, record, message_dict):
        """
        ...
        if "timestamp" not in log_record:
            now = datetime.utcnow()
            log_record["timestamp"] = datetime.strftime(now, format=LOGGING_TS_FORMAT)
        if "application" not in log_record:
            log_record["application"] = APPLICATION_NAME
